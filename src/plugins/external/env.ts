...
export default fp(
  async (fastify: FastifyInstance) => {
    ...
    const config: Config = {
      ...
      baseMapStyleUrl: raw.BASE_MAP_STYLE_URL,
      hmcrId: raw.HMCR_ID,
    ...
    if (!config.baseMapStyleUrl || config.baseMapStyleUrl.trim() === '') {
      throw new Error('BASE_MAP_STYLE_URL is required.')
    }
